...
public class Authentication {
    ...
    public static boolean isCustomer(String name, String password) throws SQLException {
        ResultSet resultSet = dataBase.executeQuery("SELECT fname from Customer WHERE email = '" + name + "' AND passW = '" + password + "'");
        return resultSet.next();
    }

    public static boolean isEmployee(String name, String password) throws SQLException {
        ResultSet resultSet = dataBase.executeQuery("SELECT fname from Employee WHERE email = '" + name + "' AND passW = '" + password + "'");
        return resultSet.next();
    }

    public static boolean isManager(String email, String password) throws SQLException {
        if (getUserType(email)!=UserType.Manager)
            return false;
        ResultSet resultSet = dataBase.executeQuery("SELECT email from Employee WHERE email = '" + email + "' AND passW = '" + password + "'");
        return resultSet.next();
    }

    public static boolean isCustomerEmail(String email) throws SQLException {
        ResultSet resultSet = dataBase.executeQuery("SELECT email from Customer WHERE email = '" + email + "';");
        return resultSet.next();
    }

    public static boolean isEmployeeEmail(String email) throws SQLException {
        if (getUserType(email)!=UserType.Employee)
            return false;
        ResultSet resultSet = dataBase.executeQuery("SELECT email from Employee WHERE email = '" + email + "';");
        return resultSet.next();
    }

    public static boolean isStore(String StoreID) throws SQLException {
        ResultSet resultSet = dataBase.executeQuery("SELECT storeId from Store WHERE storeId = '" + StoreID + "'");
        return resultSet.next();
    }
